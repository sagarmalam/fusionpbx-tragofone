...
function sc_render_header(array $session, string $active, string $title): void {
	...
	?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="color-scheme" content="light dark"><title><?= sc_escape($title) ?> · <?= sc_escape($theme['brand_name'] ?? 'Tragofone') ?></title><link rel="stylesheet" href="assets/selfcare.css"><script src="assets/selfcare.js" defer></script><style nonce="<?= sc_escape($sc_nonce) ?>">:root{--sc-bg:#<?= $values['l_bg'] ?>;--sc-fg:#<?= $values['l_fg'] ?>;--sc-button:#<?= $values['l_btn'] ?>;--sc-button-fg:#<?= $values['l_btn_fg'] ?>}@media(prefers-color-scheme:dark){:root{--sc-bg:#<?= $values['d_bg'] ?>;--sc-fg:#<?= $values['d_fg'] ?>;--sc-button:#<?= $values['d_btn'] ?>;--sc-button-fg:#<?= $values['d_btn_fg'] ?>}}</style></head><body>
