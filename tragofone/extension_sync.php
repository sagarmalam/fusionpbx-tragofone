...
<div class="tfn-shell">
	...
	<table class="tf-table"><thead><tr><th style="width:65px">Sync</th><th>Extension</th><th>FusionPBX</th><th>Self-care</th><th>Tragofone mapping</th><?php if (permission_exists('tragofone_qr_view')) { ?><th>QR login</th><?php } ?><th>Last synchronized</th></tr></thead><tbody id="extension_rows">
	<?php foreach ($extensions as $extension) { $pbx_enabled=tragofone_normalizer::boolean($extension['enabled'] ?? false); ?><tr data-search="<?= escape(strtolower(($extension['extension'] ?? '').' '.($extension['effective_caller_id_name'] ?? ''))) ?>">
		...
		<?php if ($extensions === []) { ?><tr><td colspan="<?= permission_exists('tragofone_qr_view') ? 7 : 6 ?>"><div class="tfn-empty"><b>No extensions in this domain.</b><br>Verify the selected domain or create a SIP extension under Accounts → Extensions.</div></td></tr><?php } ?>
		<?php if ($extensions !== []) { ?><tr id="extension_no_results" hidden><td colspan="<?= permission_exists('tragofone_qr_view') ? 7 : 6 ?>"><div class="tfn-empty" role="status"><b>No matching records found.</b><br>Try a different extension number or caller-ID name.</div></td></tr><?php } ?>
	</tbody></table></div>
