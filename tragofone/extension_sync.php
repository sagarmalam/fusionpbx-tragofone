...
$selected_count = 0;$selfcare_count=0;$ineligible_count=0; foreach ($extensions as &$extension) {
	try { tragofone_normalizer::sip_extension((string) $extension['extension']); $extension['sync_eligible']=true;$extension['sync_error']=null; }
	catch (InvalidArgumentException $error) { $extension['sync_eligible']=false;$extension['sync_error']=$error->getMessage();$ineligible_count++; }
...
<style>
.tf-search{min-height:40px!important;border:1px solid var(--border-color,#d0d5dd)!important;border-radius:7px!important;background:var(--input-background-color,var(--card-background-color,#fff))!important;color:inherit!important;box-shadow:none!important;outline:none}.tf-search:focus{border-color:#1570ef!important;box-shadow:0 0 0 3px rgba(21,112,239,.14)!important}
</style>
<div class="tfn-shell">
	...
	<div class="tf-alert" style="color:#344054;background:#f8fafc;border-color:#d0d5dd"><b>Tragofone extension rule:</b> only 2–15 ASCII letters or digits can be selected here. FusionPBX itself permits broader extension identifiers; unsupported records remain in FusionPBX but are never sent to Tragofone.</div>
	<div class="tf-summary"><div class="tf-stat"><strong><?= count($extensions) ?></strong><span class="tf-subtle">FusionPBX extensions</span></div><div class="tf-stat"><strong id="selected_count"><?= $selected_count ?></strong><span class="tf-subtle">Selected for Tragofone</span></div><div class="tf-stat"><strong id="excluded_count"><?= count($extensions)-$selected_count ?></strong><span class="tf-subtle">Excluded</span></div><div class="tf-stat"><strong><?= $ineligible_count ?></strong><span class="tf-subtle">Not eligible</span></div></div>
	...
	<table class="tf-table"><thead><tr><th style="width:65px">Sync</th><th>Extension</th><th>Eligibility</th><th>FusionPBX</th><th>Self-care</th><th>Tragofone mapping</th><?php if (permission_exists('tragofone_qr_view')) { ?><th>QR login</th><?php } ?><th>Last synchronized</th></tr></thead><tbody id="extension_rows">
	<?php foreach ($extensions as $extension) { $pbx_enabled=tragofone_normalizer::boolean($extension['enabled'] ?? false); ?><tr data-search="<?= escape(strtolower(($extension['extension'] ?? '').' '.($extension['effective_caller_id_name'] ?? ''))) ?>">
		...
		<td><?php if ($extension['sync_eligible']) { ?><span class="tf-badge ok">Eligible</span><?php } else { ?><span class="tf-badge" style="color:#b42318;background:#fef3f2">Not eligible</span><span class="tf-name" style="max-width:250px;color:#b42318;font-weight:600"><?= escape($extension['sync_error']) ?></span><?php } ?></td>
		...
		<?php if (permission_exists('tragofone_qr_view')) { ?><td><?php if (($extension['sync_status'] ?? '') === 'synchronized' && !empty($extension['tragofone_user_id']) && $extension['effective_sync'] && $pbx_enabled) { ?><a class="btn btn-default" href="qr_code.php?id=<?= rawurlencode($extension['extension_uuid']) ?>">Open QR</a><?php } else { ?><span class="tf-name">Available for active synchronized users</span><?php } ?></td><?php } ?>
		<td><?= escape($extension['last_synced_at'] ?: '—') ?></td>
	</tr><?php } ?>
		<?php if ($extensions === []) { ?><tr><td colspan="<?= permission_exists('tragofone_qr_view') ? 8 : 7 ?>"><div class="tfn-empty"><b>No extensions in this domain.</b><br>Verify the selected domain or create a SIP extension under Accounts → Extensions.</div></td></tr><?php } ?>
		<?php if ($extensions !== []) { ?><tr id="extension_no_results" hidden><td colspan="<?= permission_exists('tragofone_qr_view') ? 8 : 7 ?>"><div class="tfn-empty" role="status"><b>No matching records found.</b><br>Try a different extension number or caller-ID name.</div></td></tr><?php } ?>
	</tbody></table></div>
...
(function(){
	...
	document.getElementById('extension_search').addEventListener('input',function(){var q=this.value.toLowerCase().trim(),visible=0;document.querySelectorAll('#extension_rows tr[data-search]').forEach(function(row){var matches=row.getAttribute('data-search').indexOf(q)>=0;row.style.display=matches?'':'none';if(matches)visible++;});var empty=document.getElementById('extension_no_results');if(empty)empty.hidden=visible!==0;});
