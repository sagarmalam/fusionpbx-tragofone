...
<div class="tfn-shell">
	...
	<?php foreach ($extensions as $extension) { $pbx_enabled=tragofone_normalizer::boolean($extension['enabled'] ?? false); ?><tr data-search="<?= escape(strtolower(($extension['extension'] ?? '').' '.($extension['effective_caller_id_name'] ?? ''))) ?>">
		...
		<?php if (permission_exists('tragofone_qr_view')) { ?><td><?php if (($extension['sync_status'] ?? '') === 'synchronized' && $extension['effective_sync'] && $pbx_enabled) { ?><a class="btn btn-default" href="qr_code.php?id=<?= rawurlencode($extension['extension_uuid']) ?>">Open QR</a><?php } else { ?><span class="tf-name">Available for active synchronized users</span><?php } ?></td><?php } ?>
