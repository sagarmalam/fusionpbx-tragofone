...
- **My Account link is rejected:** open My Account again; launches expire in two minutes and are single-use. Verify PBX/Tragofone clocks and the global brand version if every fresh link fails. Record the displayed `SC-LAUNCH-xx` reference; it identifies the rejected validation stage without exposing the user salt, hash, URL, or session token.
