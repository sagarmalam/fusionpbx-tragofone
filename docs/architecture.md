...
Self-care adds companion-owned subjects, sessions, consumed assertions, and rate-limit buckets. A subject maps one `extension_uuid` to an encrypted salt and current global brand version. The compact Tragofone Account URL contains that opaque subject and brand version protected by a companion HMAC in addition to Tragofone's time-based MD5; global theme values remain on the PBX to satisfy the API's 200-character URL limit. Public pages derive domain, extension, mailbox, and permissions only from the authenticated subject.
