...
- Mobile voicemail downloads use an encrypted opaque capability valid for two minutes. It is accepted only while the originating session, extension, mapping, tenant, and effective access policy remain active, allowing native mobile download handlers to fetch the owned file without copying the WebView cookie.
