...
When global self-care is enabled, the worker creates or reactivates a companion subject for the extension, encrypts its unique salt, signs the global light/dark branding payload, and sends the complete Account URL through `update-configurations`. A global branding change increments the brand version, invalidates extension snapshots, and queues asynchronous configuration updates for all eligible users.

Tragofone removes the configured `tragofone_salt`, then launches the URL with `tragofone_hash=MD5(salt + time)` and `tragofone_time`. The portal verifies that hash, its own branding HMAC, the two-minute time window, single-use assertion, active extension, mapping, and tenant. It then redirects to a clean session URL.

Self-care reads the authenticated extension directly from FusionPBX. Call handling updates native `v_extensions` fields, disables conflicting DND/forward-all/Follow Me states, clears directory caches, and sends feature notification when the installed FusionPBX capability is available. Visual Voicemail resolves only the extension's mailbox and delegates playback, range handling, deletion, storage-mode support, and MWI refresh to FusionPBX's voicemail service.
