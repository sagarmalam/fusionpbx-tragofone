...
Default UI access is role-based: both `superadmin` and tenant-scoped `admin` groups can see **Advanced → Tragofone Integration**, but only `superadmin` receives Global Settings permissions. Ordinary users receive no access. Review these group assignments after installation if the deployment requires a stricter Superadmin-only policy.
