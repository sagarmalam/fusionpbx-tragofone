...
The default forwarding policy is either same-company extensions only or same-company extensions plus approved external prefixes. Use **Save Self-Care Access** after changing the global Inherit/Yes/No policy or public portal URL. **Save Global Defaults**, **Restore Theme Defaults**, and **Rotate Self-Care Salts** deliberately preserve those two access fields, preventing branding changes or a stale browser tab from disabling My Account. **Rotate Self-Care Salts** revokes active sessions and asynchronously installs new signed Account URLs for eligible users.
